...
# 2. the password. This prints a bcrypt hash; paste it into BOARD_PASSWORD_HASH verbatim.
docker run --rm caddy:2.10-alpine caddy hash-password --plaintext 'a long random string'
...
**The decision: everything behind HTTP basic auth at the proxy, and the four routes that move
money additionally pinned to a source-IP allowlist, with an SSH tunnel as the always-available
way in.**
...
three of which have money on the other end. Basic auth is the right primary gate here because the
threat is opportunistic: the internet scans every new host within minutes, and a 32-character
random password over TLS stops all of that. It has no accounts to enumerate, no session cookie to
steal, no login form to phish, and Caddy stores only a bcrypt hash. On top of it, note that every
read on this board is a `GET` and every one of the four verbs is a `POST` — so "the control
surface" is a matcher one line long, and it is additionally restricted to `BOARD_ADMIN_IPS`. The
default for that variable is `192.0.2.1`, a documentation address that is nobody, so a box you
...
**Everything returns 401 and the password is definitely right.** Almost certainly the bcrypt hash
was mangled on the way into the container. Check what Caddy actually received:
`docker compose -f compose.prod.yaml exec caddy printenv BOARD_PASSWORD_HASH` — it must start with
